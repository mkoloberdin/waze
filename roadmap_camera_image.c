...
static BOOL roadmap_camera_image_uploader( RoadMapDownloadCallbacks *callbacks, const char *image_folder, const char *image_file, char* image_id, char** message );
...
static RoadMapDownloadCallbacks gUploadCallbackFunctionsSSD =
{
...
BOOL roadmap_camera_image_upload( const char *image_folder, const char *image_file, char* image_id, char** message )
{
	...
	res = roadmap_camera_image_uploader( &gUploadCallbackFunctionsSilent,
...
static BOOL roadmap_camera_image_uploader( RoadMapDownloadCallbacks *callbacks, const char *image_folder, const char *image_file, char* image_id, char** message )
{
    ...
    // Get the full path to the file
   	fullPath = roadmap_path_join( image_folder, image_file );

    // Set the target to upload to
    target_url = roadmap_config_get ( &RMCfgCameraImageServer );

    roadmap_log( ROADMAP_DEBUG, "Uploading file: %s. ", fullPath );
    // Upload and get the response
    if ( !editor_upload_auto( fullPath, callbacks, &response_message, target_url, IMG_UPLOAD_CONTENT_TYPE ) )
    {
        // Parse the answer
        if( !strncmp( IMG_UPLOAD_RESPONSE_PREFIX, response_message, strlen( IMG_UPLOAD_RESPONSE_PREFIX ) ) )
        {
            roadmap_log( ROADMAP_WARNING, "File was uploaded successfully! Response message: %s", response_message );
            // Copy the message ID to the buffer
			memcpy( image_id, (response_message + strlen( IMG_UPLOAD_RESPONSE_PREFIX )), ROADMAP_IMAGE_ID_LEN );
			image_id[ROADMAP_IMAGE_ID_LEN] = 0;
			res = TRUE;
        }
        else
        {
            roadmap_log( ROADMAP_WARNING, "File upload error: Invalid response prefix. Response message: %s", message );
        }
    }
    else
    {
    	roadmap_log( ROADMAP_WARNING, "File upload error %s", target_url );
    }
    if ( message )
