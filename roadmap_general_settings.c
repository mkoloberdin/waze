...
#ifndef TOUCH_SCREEN
   roadmap_config_set ( &NavigateConfigNavigationGuidanceOn,
...
{
   roadmap_main_remove_periodic( on_close_event );
   ssd_dialog_free( title, FALSE );
}
...
static void on_close_dialog (int exit_code, void* context){
	...
	if (exit_code == dec_ok)
		on_ok(NULL, NULL);
#endif
	roadmap_main_set_periodic( 50L, on_close_event );
}
...
void quick_settins_exit(int exit_code, void* context){
   ...
   nav_guidance = (const char *)ssd_dialog_get_data ("nav guidance choice");
   if ( !strcmp( nav_guidance, NAV_CFG_GUIDANCE_TYPE_NATURAL ) ){
   ...
   else if ( !strcmp( nav_guidance, NAV_CFG_GUIDANCE_TYPE_TTS ) ){
   ...
   else if ( !strcmp( nav_guidance, NAV_CFG_GUIDANCE_TYPE_NONE ) ){
   ...
   roadmap_config_set ( &NavigateConfigNavigationGuidanceType, nav_guidance );
   if ( strcmp( nav_guidance, NAV_CFG_GUIDANCE_TYPE_NONE ) )
   {
...
int on_nav_guidance_selected (SsdWidget widget, const char *new_value){
...
SsdWidget create_quick_setting_menu(){
   ...
   static SsdWidget quick_container;
   int height = ssd_container_get_row_height();
   int width = ssd_container_get_width();
   const char** values = navigate_main_get_guidance_types();
   int count = navigate_main_get_guidance_types_count();
   ...
   if ( quick_container == NULL )
   {
      ...
      {
         if ( navigate_main_guidance_is_on() )
         {
         ...
         box = ssd_container_new ( "navigationguidance", NULL, SSD_MAX_SIZE, height,
                                  SSD_WIDGET_SPACE|SSD_END_ROW|tab_flag );
         ssd_widget_set_color ( box, NULL, NULL );

         box2 = ssd_container_new ( "box2", NULL, width/2, height, SSD_ALIGN_VCENTER );
         ssd_widget_set_color ( box2, NULL, NULL );
         text = ssd_text_new ( "nav guidance label", roadmap_lang_get ( "Guidance" ),
         ...
         ssd_widget_add (box,
                         ssd_choice_new ( "nav guidance choice", roadmap_lang_get (  "Guidance" ), count,
                                         labels, (const void**) values, SSD_ALIGN_RIGHT, NULL ) );

         ssd_widget_set_data( box, "nav guidance choice", current_value );
   ...
   {
      SsdWidget choice = ssd_widget_get( quick_container, "nav guidance choice" );
      ssd_choice_update_values( choice, count, labels, (const void**) values );
      current_value = navigate_main_get_guidance_type( roadmap_config_get( &NavigateConfigNavigationGuidanceType ) );
      ssd_widget_set_data( choice, "nav guidance choice", current_value );
   }
...
void roadmap_general_settings_show(void) {
   ...
   if (!( dialog = ssd_dialog_activate (title, NULL) ) ) {
...
//            ssd_widget_add(box, space(1));
         ...
         text = ssd_text_new ("prompts_label",
      ...
      if ( tts_voices_count > 0 )
      {
         ...
         ssd_widget_add (box2,
                         ssd_text_new ("tts_voices_label",
                                       roadmap_lang_get ("TTS Voices"),
                                       SSD_MAIN_TEXT_SIZE, SSD_TEXT_NORMAL_FONT|SSD_TEXT_LABEL|SSD_ALIGN_VCENTER|SSD_WIDGET_SPACE));
         ssd_widget_add(box, box2);

         ssd_widget_add (box,
                         ssd_choice_new ("TTS Voices", roadmap_lang_get ("TTS Voices"), tts_voices_count,
                                         (const char **)tts_voices_labels,
   ...
   box2 = ssd_container_new ("box2", NULL, roadmap_canvas_width()/3, SSD_MIN_SIZE,
    ...
    text = ssd_text_new ("autozoom_label",
   ...
   ssd_widget_add (container, box);
      ...
      text = ssd_text_new ("events_radius_label",
   ...
   if ( nav_guidance )
   {
      show_prompts = !strcmp( nav_guidance, NAV_CFG_GUIDANCE_TYPE_NATURAL );
      show_tts = !strcmp( nav_guidance, NAV_CFG_GUIDANCE_TYPE_TTS );
   }
   else
   {
      show_prompts = roadmap_config_match( &NavigateConfigNavigationGuidanceType, NAV_CFG_GUIDANCE_TYPE_NATURAL );
      show_tts = roadmap_config_match( &NavigateConfigNavigationGuidanceType, NAV_CFG_GUIDANCE_TYPE_TTS );
   }

   // Show/Hide prompts list
   w = ssd_widget_get( ssd_dialog_get_currently_active(), "prompts group" );
   if ( navigate_main_guidance_is_on() && show_prompts )
      ssd_widget_show( w );
   else
      ssd_widget_hide( w );
   // Show/Hide tts voices list
   w = ssd_widget_get( ssd_dialog_get_currently_active(), "TTS Voices group" );
   if ( navigate_main_guidance_is_on() && show_tts )
      ssd_widget_show( w );
   else
      ssd_widget_hide( w );
...
#ifndef TOUCH_SCREEN
      if ( navigate_main_guidance_is_on() ) pVal = yesno[0];
      else pVal = yesno[1];
