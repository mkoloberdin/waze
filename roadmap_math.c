...
static void roadmap_math_compute_scale (void) {
   ...
   if ( roadmap_screen_is_hd_screen() ) {
      RoadMapContext.zoom_x = RoadMapContext.zoom_y = RoadMapContext.zoom / 2;
   } else {
      RoadMapContext.zoom_x = RoadMapContext.zoom_y = RoadMapContext.zoom;
   }
...
int roadmap_math_set_scale (int scale, int use_map_units) {
   ...
   if ( roadmap_screen_is_hd_screen() ) {
      return roadmap_math_zoom_set (res * 2);
   } else {
...
#if 0 && defined(OPENGL) && defined(VIEW_MODE_3D_OGL)
		if (RoadMapContext._is3D_projection == PROJECTION_MODE_3D) {
			int hdist;
			int vdist;
			int dist;
