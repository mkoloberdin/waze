...
int RealtimeAlertCommentsList(int iAlertId)
{
   ...
   while (iCount < MAX_COMMENTS_ENTRIES && (CommentEntry != NULL))
   {
       ...
       if (CommentEntry->comment.bCommentByMe)
          ...
          bitmap = ssd_bitmap_new("CommentSent", "response_bubble1",SSD_END_ROW|SSD_ALIGN_RIGHT );
	   ...
	   if (CommentEntry->comment.bCommentByMe){
		...
		if ( roadmap_screen_is_hd_screen() )
		  text_container = ssd_container_new("text_con", NULL, 320, 100, 0 );
		else
          text_container = ssd_container_new("text_con", NULL, 205, 80, 0 );
	   }
       ...
       if (CommentEntry->comment.iMood != 0){
         ...
         if ( roadmap_screen_is_hd_screen() )
            mood_con = ssd_container_new("mood_Cont", NULL, 48, 48, SSD_ALIGN_RIGHT|SSD_ALIGN_BOTTOM );
         else
       ...
       if ( roadmap_screen_is_hd_screen() )
         ...
         container = ssd_container_new("AddCommentCont", NULL, SSD_MAX_SIZE, 40, 0 );

       ssd_widget_set_color(container, NULL, NULL);
       snprintf(CommentStr + strlen(CommentStr), sizeof(CommentStr)
               - strlen(CommentStr), "  %s",
               CommentEntry->comment.sDescription);
       text = ssd_text_new("CommnetTxt", CommentStr, -1,SSD_TEXT_NORMAL_FONT);
       ...
       if (CommentEntry->comment.bCommentByMe){
          ...
          icon[2] = NULL;
          button = ssd_button_new( "Reprt_Abuse_Button", "ReportAbuse", (const char**) &icon[0], 2, SSD_ALIGN_RIGHT|SSD_ALIGN_VCENTER, on_report_abuse );
