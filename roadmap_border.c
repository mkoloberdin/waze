...
static const char* get_img_filename( border_images image_id)
{
   switch( image_id)
   {
   	  ...
   	  case border_image_bottom_no_frame2: return "Bottom_NoFrame";
...
static RoadMapImage create_sides_image (int style, int header, int pointer_type, RoadMapGuiPoint *bottom, RoadMapGuiPoint *top, int num_items) {
   RoadMapImage image;
   int image_width = bottom->x  - top->x;
   RoadMapGuiPoint point;
   int i;
   
   image = roadmap_canvas_new_image (image_width, s_images[border_image_left+style].height * num_items);

   for (i = 0; i<num_items; i++){
      point.x = image_width - s_images[border_image_right+style].width ;
      point.y = i* s_images[border_image_right+style].height;
      roadmap_canvas_copy_image (image, &point, NULL, s_images[border_image_right+style].image, IMAGE_NORMAL);


      point.x = 0;
      point.y = i* s_images[border_image_left+style].height;
      roadmap_canvas_copy_image (image, &point, NULL, s_images[border_image_left+style].image, IMAGE_NORMAL);
   }
...
static RoadMapImage create_top_image (int style, int header, int pointer_type, RoadMapGuiPoint *bottom, RoadMapGuiPoint *top) {
   ...
   image = roadmap_canvas_new_image (image_width, s_images[border_image_top+style].height);
...
#endif      
      right_point.x = image_width - s_images[border_image_right+style].width;
      left_point.y = 0;
      
      point.x = 0;
      point.y = 0;
      roadmap_canvas_copy_image (image, &point, NULL, s_images[border_image_top_left+style].image, IMAGE_NORMAL);


       point.x = right_point.x;
       point.y = 0;
       roadmap_canvas_copy_image (image, &point, NULL, s_images[border_image_top_right+style].image, IMAGE_NORMAL);
 
      
      num_items = (image_width - s_images[border_image_right+style].width - s_images[border_image_left+style].width)/s_images[border_image_top+style].width;
      for (i = 0; i<num_items; i++){
         point.x = s_images[border_image_top_left+style].width + i * s_images[border_image_top+style].width ;
         point.y = 0;
         roadmap_canvas_copy_image (image, &point, NULL, s_images[border_image_top+style].image, IMAGE_NORMAL);
      }
...
static RoadMapImage create_top_header_image (int style, int header, int pointer_type, RoadMapGuiPoint *bottom, RoadMapGuiPoint *top) {
   ...
   image = roadmap_canvas_new_image (image_width, s_images[header].height);
      ...
      point.x = 0;
      point.y = 0;
      roadmap_canvas_copy_image (image, &point, NULL, s_images[header].image, IMAGE_NORMAL);


       point.x = right_point.x;
       point.y = 0;
       roadmap_canvas_copy_image (image, &point, NULL, s_images[header+2].image, IMAGE_NORMAL);
      ...
      for (i = 0; i<num_items; i++){
         ...
         point.y = 0;
         roadmap_canvas_copy_image (image, &point, NULL, s_images[header+1].image, IMAGE_NORMAL);
      }
...
static RoadMapImage create_bottom_image (int style, int header, int pointer_type, RoadMapGuiPoint *bottom, RoadMapGuiPoint *top) {
   ...
   image = roadmap_canvas_new_image (image_width, s_images[border_image_top+style].height);
      ...
      point.x = 0;
      point.y = 0;
      roadmap_canvas_copy_image (image, &point, NULL, s_images[border_image_bottom_left+style].image, IMAGE_NORMAL);


       point.x = right_point.x;
       point.y = 0;
       roadmap_canvas_copy_image (image, &point, NULL, s_images[border_image_bottom_right+style].image, IMAGE_NORMAL);
 
      
      num_items = (image_width - s_images[border_image_right+style].width - s_images[border_image_left+style].width)/s_images[border_image_top+style].width;
      for (i = 0; i<num_items; i++){
         point.x = s_images[border_image_top_left+style].width + i * s_images[border_image_bottom+style].width ;
         point.y = 0;
         roadmap_canvas_copy_image (image, &point, NULL, s_images[border_image_bottom+style].image, IMAGE_NORMAL);
      }
...
static RoadMapImage get_image (int side, int style, int header, int pointer_type, RoadMapGuiPoint *bottom, RoadMapGuiPoint *top, int num_items){
   ...
   for (i = 0; i < border_cached_table.count; i++){
       if ( (border_cached_table.resource[i].style == style) && 
            (border_cached_table.resource[i].header == header) && 
            (border_cached_table.resource[i].side == side) && 
            (border_cached_table.resource[i].num_items == num_items) && 
            (border_cached_table.resource[i].width == image_width) ){
          return border_cached_table.resource[i].image;
       }
   }
   ...
   if (index == MAX_CACHE-1){
      return create_image(side, style, header, pointer_type, bottom, top, num_items);
   }
	   ...
	   image = get_image(border_top, style, header, pointer_type, bottom, top, 0);
		...
		image =  get_image(border_top, style, header, pointer_type, bottom, top, 0);
  		...
  		if (style == STYLE_BLACK)
  		   top_height = s_images[border_image_top+style].height+7;
  		else
	...
	if (pointer_type == POINTER_NONE){
	   ...
	   roadmap_canvas_draw_image (s_images[border_image_bottom_left+style].image, &left_point, 0, IMAGE_NORMAL);
		...
		if (pointer_type == POINTER_MENU)
			...
			num_items = (right_point.x - left_point.x- 100)/s_images[border_image_bottom+style].width;
		else
		...
		if (pointer_type == POINTER_POSITION){
			...
			if (visible){
				...
				points[1].x = point.x;
				points[1].y = point.y+s_images[border_image_bottom_no_frame+2].height-3;
				points[2].x = point.x+42;
				points[2].y = point.y+s_images[border_image_bottom_no_frame+2].height-3;
...
//				roadmap_canvas_draw_multiple_polygons (1, &count, points, 0, 0);
    			...
    			for (i=0; i<40; i++){
    				...
    				point.x = point.x + s_images[border_image_bottom_no_frame+2].width ;
					point.y = point.y;
					roadmap_canvas_draw_image (s_images[border_image_bottom_no_frame+2].image, &point, 0, IMAGE_NORMAL);
    			}
				}
	...
	if ((style == STYLE_NORMAL) || (style == STYLE_BLACK)){
	   if (pointer_type == POINTER_POSITION)
		...
		fill_points[2].x = left_point.x + s_images[border_image_left+style].width ;
		fill_points[2].y = top->y + top_height - 7;
		fill_points[3].x =left_point.x + s_images[border_image_left+style].width ;
