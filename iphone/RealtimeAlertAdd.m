...
- (void) sendAlert {
	...
	const char *path = roadmap_path_first ("config");
	const char *file_name = IMAGE_FILENAME;
	char image_id[ROADMAP_IMAGE_ID_BUF_LEN];
   ...
   if (roadmap_file_exists(path, file_name))
      roadmap_camera_image_upload( path, file_name, image_id, NULL );
   else
      image_id[0] = 0;
	
	//Send report
	success = Realtime_Report_Alert(alert_type, description, alert_direction, image_id, roadmap_twitter_is_sending_enabled() );
