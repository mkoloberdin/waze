...
static void show_languages () {
   ...
   for (i = 0; i < lang_count; ++i) {
      ...
      if (strcmp(lang_values[i], roadmap_lang_get_system_lang()) == 0) {
   ...
   text = [NSString stringWithUTF8String:roadmap_lang_get ("Language")];
	langView = [[RoadMapChecklist alloc] activateWithTitle:text andData:dataArray andHeaders:NULL andCallback:language_callback];
}
...
static void show_prompts() {
   ...
   text = [NSString stringWithUTF8String:roadmap_lang_get ("Prompts")];
	promptsView = [[RoadMapChecklist alloc] activateWithTitle:text andData:dataArray andHeaders:NULL andCallback:prompts_callback];
...
- (void) populateData
{
   ...
   //Backlight
	...
	//Units
   //if (strcmp(roadmap_prompts_get_name(),"heb")) {
