                                 ...
                                 roadmap_result       rc)
{
      ...
      if( is_network_error( rc))
         ...
         roadmap_messagebox_cb ( roadmap_lang_get( "Resolve Address"),
                                roadmap_lang_get( "String could not be resolved\r\n"
                                                 "to a valid address"), on_search_error_message );
      else
      {
         char msg[64];
   ...
   indexes[i] = (void*)(COULDNT_FIND_INDEX +10);
   icons[i] = "submit_logs";
	
	roadmap_list_menu_generic(roadmap_lang_get(ASD_DIALOG_TITLE), size+1, results, (void *)indexes, 
							  icons, NULL, NULL, on_list_item_selected, NULL, NULL, NULL, on_options, 60, 0, TRUE);
}
...
int on_options(SsdWidget widget, const char *new_value, void *context)	
{
	...
	//static   const char* icons[MAX_MENU_ITEMS];
	int i;
   int count;
   
   selected_item = (int)new_value - 10;
