                                 ...
                                 roadmap_result       rc)
{
   ...
   size++;
   results[0] = strdup("Local search results");
   indexes[0] = NULL;
   icons[0] = "google_logo";
   for( i=1; i< size; i++)
   {
      results[i] = array[i-1].address;
      indexes[i] = (void*)(i+10); //TODO: this seems like bad idea, need to find better solution for the id.
	   icons[i] = "search_local";
   }
	
	roadmap_list_menu_generic(roadmap_lang_get(LSD_DIALOG_TITLE), size, results, (void *)indexes, 
							  icons, NULL, NULL, on_list_item_selected, NULL, NULL, NULL, on_options, 90, 0, TRUE);
}
...
int on_options(SsdWidget widget, const char *new_value, void *context)	
{
   ...
   selected_item = (int)new_value - 10 - 1;
    ...
    */
	...
	roadmap_list_menu_generic("Options", count, labels, (const void**)values, NULL, NULL, NULL, on_option_selected, NULL, NULL, NULL, NULL, 60, 0, FALSE);
                              ...
                              void*                context)
{
   ...
   if( !s_history_was_loaded)
   {
	...
	ShowSearchbox (roadmap_lang_get ("Local search"), roadmap_lang_get ("Enter your search"), on_searchbox_done, NULL);
}
