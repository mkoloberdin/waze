...
class CFreeMapAppUi : public CAknAppUi, public MRemConCoreApiTargetObserver
{
	public: // Constructors and destructor
		...
		*/
		...
		/**
		 * Checks if software or hardware qwerty is available
		 */
		inline TBool IsQwertyEnabled( void );
		/**
		 * Checks if hardware qwerty is available
		 */
		inline TBool IsHwQwertyEnabled( void );
	...
	private: // Data
		...
		*/
		...
		CProfilerTimer* m_pProfilerTimer;
...
// CLASS DECLARATION
/**
* CProfilerTimer intended to provide periodic sampling of the resources allocated 
* to the application
*/
class CProfilerTimer : public CTimer
{
	public: 
		static CProfilerTimer* NewL();
		void ConstructL();
		void Start( TInt aInterval /* In milliseconds */ );	
	
		virtual ~CProfilerTimer();
		
	protected:
		
		CProfilerTimer( TInt aPriority  );
		virtual void RunL( void );
		virtual void DoCancel( void );
		virtual TInt RunError( TInt aError );
